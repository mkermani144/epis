...
use crate::lingoo::commands::chat::commands::{
  new_chat::handle_lingoo_new_chat, rename_chat::handle_lingoo_rename_chat,
  resume_chat::handle_lingoo_resume_chat,
};
...
pub async fn handle_lingoo_chat(command: LingooChatCommand) -> Result<()> {
  match command {
    ...
    LingooChatCommand::Rename => handle_lingoo_rename_chat().await,
  }
}
